...
def renumber_nodes_and_edges(g):
    g['eid'] = np.arange(0,g['eid'].shape[0])
    return g
...
def to_dgl_blocks(ret, hist, reverse=False, cuda=True,device_id=0):
    mfgs = list()
    for r in ret:
        if not reverse:
            b = dgl.create_block((r.col(), r.row()), num_src_nodes=r.dim_in(), num_dst_nodes=r.dim_out()) # coo格式
        ...
        if cuda:
            mfgs.append(b.to(torch.device('cuda',device_id)))
        else:
            mfgs.append(b)
...
def node_to_dgl_blocks(root_nodes, ts, cuda=True,device_id=0):
    mfgs = list()
    ...
    if cuda:
        mfgs.insert(0, [b.to(torch.device('cuda',device_id))])
    else:
        mfgs.insert(0, [b])
    return mfgs
...
def prepare_input2(mfgs, node_feats, edge_feats, combine_first=False, nfeat_buff=None, efeat_buff=None,disable_edge=False, disable_node=False,device_id=0):
    # print(f"disable_edge: {disable_edge}, disable_node: {disable_node}")
    if combine_first:
        for i in range(len(mfgs[0])):
            if mfgs[0][i].num_src_nodes() > mfgs[0][i].num_dst_nodes():
                num_dst = mfgs[0][i].num_dst_nodes()
    ...
    if node_feats is not None:
        for b in mfgs[0]:
            if not disable_node:
                b.srcdata['h'] = nfeat_buff.Get(b.srcdata['ID'].long())
            else:
                b.srcdata['h'] = node_feats[b.srcdata['ID'].long().cpu()].cuda(device_id)
    i = 0
    if edge_feats is not None:
        for mfg in mfgs:
            for b in mfg:
                if b.num_src_nodes() > b.num_dst_nodes():
                    if not disable_edge:
                        b.edata["f"] = efeat_buff.Get(b.edata["ID"].long())
                    else:
                        b.edata["f"] = edge_feats[b.edata["ID"].long().cpu()].cuda(device_id)
    return mfgs
...
class EarlyStopMonitor(object):
  def __init__(self, max_round=3, higher_better=True, tolerance=1e-8):
