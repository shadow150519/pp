...
parser.add_argument("--partition_interval",type=int,default=10)
parser.add_argument("--n_warmup_epoch",type=int,default=10)
...
# for e in range(args.n_warmup_epoch):
#  # torch.cuda.empty_cache()
#     print('warmup Epoch {:d}:'.format(e))
#     time_sample = 0
#     time_prep = 0
#     time_prep_mail = 0
#     time_tot = 0
...
for e in range(n_epoch):
    if e % args.partition_interval == 0:
        ...
        mp_list = []
        train_edge_cnt = 0
        mp = MergePartition([partitions[pid] for pid in range(pg.partition_num())], False)
        mp_list.append(mp)
        train_edge_cnt += len(mp)
        print(f"edge num:{pg.edge_num()}, train edge num:{train_edge_cnt}, train ratio:{train_edge_cnt/pg.edge_num():.3f}")
        ppd = PP2Dataset(mp_list)
    ...
    for i, rows in enumerate(edges):
        ...
        # 前向包括update memory和embedding
        pred_pos, pred_neg = model(mfgs)
        pg.update_edge_weights(rows[2],pred_pos)
        time_forward += time.time() - t_forward
