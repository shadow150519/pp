...
early_stop = EarlyStopMonitor(args.tolerance)
...
for e in range(n_epoch):
    ...
    drop_ratio = float(args.drop_rate)
    # batch训练
    ...
    for i, (_, rows) in enumerate(df[:train_edge_end].groupby(group_indexes[random.randint(0, len(group_indexes) - 1)])):
        ...
        # 从 `rows` 中随机抽取 retain_ratio 的行数，并保持原顺序
        retained_rows = rows.sample(frac=retain_ratio, random_state=123).sort_index()
        if need_profile:
            prof.step()
            if i > 3+ 2 +20:
                prof.stop()
